...
def simple_edit_video(filename, operations, new_filename, video_status):
    try:
        ...
        update_progress(tmp_filename, video_status, process, nb_frame)
    except:
        video_status.update({tmp_filename: -1})
...
def put_watermark(video_filename, image_filename, new_filename, video_status, operations):
    ...
    try:
        ...
        process = subprocess.Popen(['ffmpeg', '-y', '-i', video_filename, '-i', image_filename,
                                       '-filter_complex', f"[1][0]scale2ref=h=ow/mdar:w=iw/4[#A logo][movie];[#A logo]format=argb,colorchannelmixer=aa=0.5[#B logo transparent];[movie][#B logo transparent]overlay={position}", tmp_filename], stdout=subprocess.PIPE, stderr=subprocess.STDOUT,universal_newlines=True)
